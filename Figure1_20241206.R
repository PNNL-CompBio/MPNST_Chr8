...
ggplot2::ggplot(med.chr8q, aes(x=Sample, y=`Median Chr8q Copy Number`, fill = Omics)) + 
  ...
  #scale_fill_manual(values = c("#00BFC4", "#F8766D"))
#scale_fill_manual(values = c("black", "grey"))
ggplot2::ggsave(paste0("PDX Copy Number_Chr8q_median_differentLegend", Sys.Date(), ".pdf"), width = 7, height = 7)
...
mean.dot.df <- read.csv(synapser::synGet("syn64024301")$path)
# topGenes <- na.omit(mean.dot.df[(grepl("RNA", mean.dot.df$sig_types) | grepl("Protein", mean.dot.df$sig_types)) &
#                                   grepl("RNA, Protein", mean.dot.df$types),]$feature) # 178
# topGenes <- na.omit(mean.dot.df[grepl("RNA, Protein", mean.dot.df$sig_types),]$feature) # 6
# topGenes <- na.omit(mean.dot.df[(grepl("RNA", mean.dot.df$sig_types) & grepl("Protein", mean.dot.df$sig_types)),]$feature) # 178
...
#topGenes <- unique(dot.df[dot.df$Significant & abs(dot.df$Spearman.est) >= 0.9,]$Feature) # 1016
top.mean.df <- mean.dot.df[mean.dot.df$feature %in% topGenes,]
geneOrder <- top.mean.df[order(top.mean.df$mean_rankVal),]$feature
dot.df <- all.degs[all.degs$Feature %in% topGenes,]
if (any(dot.df$Spearman.q == 0)) {
