...
##### pick dose range? change min dose to 0.0001 instead of 4 from coderdata so now range is 0.0001 to 10 uM for fit_auc #####
fit_auc <- confluence[confluence$dose_response_metric=="fit_auc",]
fit_auc.fish <- merge(fit_auc, fish, by="improve_sample_id")
Drug <- unique(fit_auc$improve_drug_id) # 5
t.df <- data.frame(Drug=sort(rep(Drug,2)), Pearson.est=NA, Pearson.p=NA, 
                   Spearman.est=NA, Spearman.p=NA, 
                   test=rep(c("MYC","chr8"),length(Drug)))

for (d in Drug) {
  temp.fit_auc.fish <- fit_auc.fish[fit_auc.fish$improve_drug_id==d,]
  for (t in c("chr8","MYC")) {
    temp.fit_auc.fish$rank <- as.numeric(temp.fit_auc.fish[,t])
    temp.fit_auc.fish <- na.omit(temp.fit_auc.fish) # remove NF11.1 because not sure exact copy number or rank
    temp.test <- cor.test(temp.fit_auc.fish$rank,temp.fit_auc.fish$dose_response_value)
    temp.test.sp <- cor.test(temp.fit_auc.fish$rank,temp.fit_auc.fish$dose_response_value, method="spearman")
    ...
    if (is.numeric(temp.test$p.value) | is.numeric(temp.test.sp$p.value)) { # was checking for p < 0.05 but now want plots either way
      ...
      ggplot(temp.fit_auc.fish, aes(x=rank, y=dose_response_value)) + geom_point() + theme_minimal() + 
        labs(x=paste(t,"Copy Number"), y = paste(d,"Sensitivity (fit_auc)")) +
        geom_smooth(se=FALSE, linetype="dashed", method="lm") + 
        ggrepel::geom_label_repel(aes(label=improve_sample_id)) + 
        ggplot2::geom_text(
          x = Inf, y = -Inf, vjust = "inward", hjust = "inward",
...
write.csv(t.df, "correlations_fit_auc_chr8_MYC_2025-07-28.csv", row.names=FALSE) # none significant

# try plotting PLKi and EGFRi together and facet by chr8/MYC copy number
fit_auc.fish$moa <- NA
fit_auc.fish[fit_auc.fish$improve_drug_id %in% c("BI-2536","Volasertib"),]$moa <- "PLK Inhibitor"
fit_auc.fish[fit_auc.fish$improve_drug_id %in% c("Erlotinib","Osimertib"),]$moa <- "EGFR Inhibitor"
#fit_auc.fish[fit_auc.fish$improve_drug_id %in% c("Mirdametinib"),]$moa <- "MEK Inhibitor"
moa <- na.omit(unique(fit_auc.fish$moa)) # 5
t.df <- data.frame(moa=sort(rep(moa,2)), Pearson.est=NA, Pearson.p=NA, 
                   Spearman.est=NA, Spearman.p=NA, 
                   test=rep(c("MYC","chr8"),length(moa)))
for (m in moa) {
  temp.fit_auc.fish <- fit_auc.fish[fit_auc.fish$moa==m,]
  for (t in c("chr8","MYC")) {
    temp.fit_auc.fish$rank <- as.numeric(temp.fit_auc.fish[,t])
    temp.fit_auc.fish <- na.omit(temp.fit_auc.fish) # remove NF11.1 because not sure exact copy number or rank
    temp.test <- cor.test(temp.fit_auc.fish$rank,temp.fit_auc.fish$dose_response_value)
    ...
    if (is.numeric(temp.test$p.value) | is.numeric(temp.test.sp$p.value)) { # was checking for p < 0.05 but now want plots either way
      ...
      ggplot(temp.fit_auc.fish, aes(x=rank, y=dose_response_value)) + geom_point() + theme_minimal() + 
        labs(x=paste(t,"Copy Number"), y = paste(m,"Sensitivity (fit_auc)")) +
        geom_smooth(se=FALSE, linetype="dashed", method="lm") + 
        ggrepel::geom_label_repel(aes(label=improve_sample_id)) + 
        ggplot2::geom_text(
          x = Inf, y = -Inf, vjust = "inward", hjust = "inward",
...
write.csv(t.df, "moa_correlations_fit_auc_chr8_MYC_2025-07-28.csv", row.names=FALSE) # none significant

# is PLKi fit_auc lower with MYC gain? is EGFRi fit_auc lower with MYC diploid? Likewise with chr8
fit_auc$chr8 <- "Gain"
fit_auc[fit_auc$improve_sample_id %in% c("JH-2-055d","ST8814","NF10.1"),]$chr8 <- "Diploid"
fit_auc$chr8q <- "Gain"
fit_auc[fit_auc$improve_sample_id %in% c("JH-2-055d","ST8814","NF10.1"),]$chr8q <- "Diploid"
fit_auc$MYC <- "Gain"
fit_auc[fit_auc$improve_sample_id == "JH-2-055d",]$MYC <- "Diploid"
Drug <- unique(fit_auc$improve_drug_id) # 5
t.df <- data.frame(Drug=sort(rep(Drug,2)), p=NA, mean_gain = NA, median_gain=NA, sd_gain=NA, N_gain=NA,
                   mean_diploid = NA, median_diploid=NA, sd_diploid=NA, N_diploid=NA, test=rep(c("MYC","chr8"),length(Drug)),
                   gain_moreSensitive=FALSE)
...
for (d in Drug) {
  for (t in c("chr8","MYC")) {
    gain <- na.omit(fit_auc[fit_auc[,t]=="Gain" & fit_auc$improve_drug_id==d,]$dose_response_value)
    diploid <- na.omit(fit_auc[fit_auc[,t]=="Diploid" & fit_auc$improve_drug_id==d,]$dose_response_value)
    if (length(diploid) > 1) {
      if (mean(gain) > mean(diploid)) {
        temp.test <- t.test(gain, diploid, "greater")
      } else {
...
write.csv(t.df, "tTests_fit_auc_chr8_MYC.csv", row.names=FALSE)
# lowest p-value is 0.0874 (all for chr8; can't do MYC because only 1 MYC diploid)

# repeat with moa instead of drug
fit_auc$moa <- NA
fit_auc[fit_auc$improve_drug_id %in% c("BI-2536","Volasertib"),]$moa <- "PLK Inhibitor"
fit_auc[fit_auc$improve_drug_id %in% c("Erlotinib","Osimertinib"),]$moa <- "EGFR Inhibitor"
#fit_auc[fit_auc$improve_drug_id %in% c("Mirdametinib"),]$moa <- "MEK Inhibitor"
moa <- na.omit(unique(fit_auc$moa)) # 5
t.df <- data.frame(moa=sort(rep(moa,2)), p=NA, mean_gain = NA, median_gain=NA, sd_gain=NA, N_gain=NA,
                   mean_diploid = NA, median_diploid=NA, sd_diploid=NA, N_diploid=NA, test=rep(c("MYC","chr8"),length(moa)),
                   gain_moreSensitive=FALSE)
...
for (m in moa) {
  for (t in c("chr8","MYC")) {
    gain <- na.omit(fit_auc[fit_auc[,t]=="Gain" & fit_auc$moa==m,]$dose_response_value)
    diploid <- na.omit(fit_auc[fit_auc[,t]=="Diploid" & fit_auc$moa==m,]$dose_response_value)
    if (length(diploid) > 1) {
      if (mean(gain) > mean(diploid)) {
        temp.test <- t.test(gain, diploid, "greater")
      } else {
...
write.csv(t.df, "moa_tTests_fit_auc_chr8_MYC.csv", row.names=FALSE)

temp.df <- reshape2::melt(fit_auc, id.var=c("source", "improve_sample_id", 
                                        "improve_drug_id", "study", "time",
                                        "time_unit", "dose_response_metric",
                                        "dose_response_value","moa"))
temp.df <- temp.df[temp.df$variable != "chr8q" & temp.df$moa != "MEK Inhibitor",]
...
ggplot(temp.df, aes(x=value, y=dose_response_value)) + geom_violin(alpha=0) + 
  ...
  facet_grid(moa~variable) + theme_classic() + labs(color="Drug", shape="MPNST Cell Line", y ="Viability Area Under the Curve") +
  scale_shape_manual(values=1:length(all.mpnst), breaks=all.mpnst) + theme(axis.title.x=element_blank())
ggsave("fit_auc_vs_moa_boxPlot.pdf",width=4, height=4.5)
...
ggplot(temp.df, aes(x=value, y=dose_response_value)) + geom_violin(alpha=0) + 
  ...
  theme(axis.title.y=element_blank(), legend.position="bottom") + coord_flip() + 
  scale_x_discrete(position="top")
ggsave("fit_auc_vs_moa_boxPlot_horizontal.pdf",width=4.5, height=3)
ggsave("fit_auc_vs_moa_boxPlot_horizontal_wider.pdf",width=6, height=3)
...
ggplot(temp.df, aes(x=value, y=dose_response_value)) + geom_violin(alpha=0) + 
  ...
  facet_grid(Drug~variable) + theme_classic() + labs(color="MOA", shape="MPNST Cell Line", y ="Viability Area Under the Curve") +
  scale_shape_manual(values=1:length(all.mpnst), breaks=all.mpnst) + theme(axis.title.x=element_blank())
ggsave("fit_auc_vs_drug_boxPlot_v2.pdf",width=4, height=6)
ggsave("fit_auc_vs_drug_boxPlot_v2_shorter.pdf",width=4, height=5)
...
ggplot(temp.df, aes(x=value, y=dose_response_value)) + geom_violin(alpha=0) + 
  ...
  theme(axis.title.y=element_blank(), legend.position="bottom") + coord_flip() + 
  scale_x_discrete(position="top")
ggsave("fit_auc_vs_drug_boxPlot_horizontal.pdf",width=6, height=3)
ggsave("fit_auc_vs_drug_boxPlot_horizontal_wider.pdf",width=8, height=3)
write.csv(temp.df,"fit_auc_vs_drug_or_moa.csv", row.names=FALSE)
