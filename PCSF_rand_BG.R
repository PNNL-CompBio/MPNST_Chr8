...
#' @param terminals  A list of lists of terminal genes with prizes to be analyzed in the PCSF context. 
...
function(ppi, terminals, n = 10, r = 0.1, w = 2, b = rep(1, length(terminals)), 
  ...
  if (missing(terminals)){
    stop("  Need to provide terminal nodes in a named list of named numeric vectors, 
    where node names must be same as in the interaction network.")
  }
  if(is.null(names(terminals))) {
    stop("  The terminal nodes must be provided in a named list of named numeric vectors, 
    where node names must be same as in the interaction network.")
  }
  if (length(b) != length(terminals)) {
    stop("  Length of terminals list must match length of b vector")
  }
