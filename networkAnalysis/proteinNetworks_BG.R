...
#' @param vals.list list of named lists of gene values representing protein data
...
computeProteinNetwork_BG<-function(vals=list(),nrand=100,
  ...
  ##assign node types
  val.types <- names(vals)
  types.so.far <- c()
  for (i in 1:length(val.types)) {
    types[intersect(names(V(subnet)),names(vals[[i]]))]<-val.types[i]
    ...
    if (length(other.types) > 0) {
      for (j in 1:length(other.types)) {
        temp.other.vals <- names(vals[other.types[j]])
        types[intersect(names(V(subnet)),
                        intersect(names(vals[[i]]),temp.other.vals))] <- 
          paste0(types[intersect(names(V(subnet)),
                                 intersect(names(vals[[i]]),temp.other.vals))],
                 "+",other.types[j])
      } 
    }
  ...
  subnet <- igraph::set.vertex.attribute(subnet, 
                                         "name", 
                                         value=names(V(subnet)))
  
  subnet<-igraph::set.vertex.attribute(subnet,'inputPrize',value=lfcs)
...
#' @import remotes
#' @export
computePhosphoNetwork_BG<-function(phos.vals=c(),prot.vals=c(),
                                gene.vals=c(),rna.vals=c(),
                                nrand=100,beta=2,mu=0.0005,fname){
  ...
  kdat<-KSDB%>%group_by(GENE)%>%dplyr::select(SUB_GENE,SUB_MOD_RSD)%>%
    rowwise()%>%
 ...
 #  print(adf)
   ...
   subnet<-NULL
   try(
     subnet <- PCSF_rand(ppi,abs(terms), n=nrand, r=0.2,w = 4, b = beta, mu = mu)
   )
  ...
  #types[intersect(names(V(subnet)),intersect(names(gene.vals),names(rna.vals)))] <- 'mutation+transcriptomic'
  #types[intersect(names(V(subnet)),intersect(names(gene.vals),names(prot.vals),names(rna.vals)))] <- 'mutation+transcriptomic+proteomic'
