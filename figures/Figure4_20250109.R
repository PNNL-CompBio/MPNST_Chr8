...
pos.net.centr <- read.csv(file.path("~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/analysis/Chr8_quant_20250409",
                                "positive_1PlusConnection_centrality.csv"))
...
neg.net.centr <- read.csv(file.path("~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/analysis/Chr8_quant_20250409",
                                    "negative_1PlusConnection_centrality.csv"))
...
excluded.targets <- drug.targets[!(drug.targets %in% other.features)] # 1: CA7
...
for (j in toxic) {
  ...
  if (nrow(filtered.percScores) > 0) {
    ...
    moa.plot <- ggplot2::ggplot(filtered.percScoresInfo, aes(x=Target,y=1, fill=moa)) + 
      ...
      scale_fill_manual(values=myColorPal, breaks=names(myColorPal)) +
