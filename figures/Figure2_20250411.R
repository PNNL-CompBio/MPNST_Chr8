...
all.gsea <- rbind(gsea.cn, gsea.rna.hall, gsea.rna.kegg, gsea.rna.onc,
                  gsea.prot.hall, gsea.prot.kegg, gsea.prot.onc)
...
gsea.rna.prot <- all.gsea[all.gsea$Omics %in% c("RNA", "Protein") &
                            all.gsea$Collection %in% c("Hallmark", "KEGG", "Oncogenic_signatures"),]
...
collections <- c("Oncogenic", "Hallmark", "KEGG")
...
for (onc in dup.sig.onc.info) {
  ...
  rel.gs[[onc]] <- c(rel.hall, rel.kegg)
...
all.rel.gs <- unlist(rel.gs, use.names=TRUE) # 44 MAP2K pathways, 36 KRAS, 12 MTOR, 10 IL2, 1 each of DCA/DCAF, RPS14, ESR1 
all.rel.gs2 <- unique(unlist(rel.gs))
sig.rel.gs <- unique(gsea.rna.prot[gsea.rna.prot$Feature_set %in% all.rel.gs2 & gsea.rna.prot$Significant,]$Feature_set) # 8
sig.rel.gs.df <- gsea.rna.prot[gsea.rna.prot$Feature_set %in% all.rel.gs2 & gsea.rna.prot$Significant,] # 8
sig.rel.gs.short <- sub("KEGG_", "", sig.rel.gs)
sig.rel.gs.short <- sub("HALLMARK_", "", sig.rel.gs.short)
...
for (i in collections) {
  ...
  temp.mean.gsea$Feature_set <- sub("KEGG_", "", temp.mean.gsea$Feature_set)
  ...
  temp.dot.df$Feature_set <- sub("KEGG_", "", temp.dot.df$Feature_set)
...
gsea.dot.plots
ggplot2::ggsave("Enriched_geneSets_dotPlot_patchworkCollection_boldOnc_boldTitles_25max.pdf", gsea.dot.plots, width=6, height=13)
...
onc.dot.plots <- NULL
all.topGenes <- c()
for (onc in c("MAP2K", "KRAS")) { # maybe add MTOR, IL2 
  onc.gene <- ifelse(onc == "MAP2K", "MAP2K1", "KRAS")
  myc.hallmark <- unique(hall.info[hall.info$gs_name %in% 
                                     rel.gs[[onc]],]$gene_symbol)
  myc.kegg <- unique(kegg.info[kegg.info$gs_name %in% 
                                 rel.gs[[onc]],]$gene_symbol)
  myc.genes <- unique(c(myc.hallmark, myc.kegg))#, onc.gene))
  ...
  dot.df <- dot.df %>% slice_max(abs(Spearman.est), n=25)
  ...
  myc.dot.plot <- ggplot2::ggplot(
    ...
    theme(axis.title.x=element_blank(), axis.title.y=element_blank()) + ggtitle(onc) +
    theme(plot.title = element_text(hjust = 0.5, face="bold", size=16)) +
    theme(axis.text.y=element_text(face=geneFace), axis.text.x=element_text(angle=45, vjust=1, hjust=1))
  if (is.null(onc.dot.plots)) {
    onc.dot.plots <- myc.dot.plot
  } else {
    onc.dot.plots <- onc.dot.plots / myc.dot.plot
  }
}
dup.topGenes <- all.topGenes[duplicated(all.topGenes)]
onc.dot.plots <- onc.dot.plots + plot_layout(guides="collect")
#ggplot2::ggsave("Correlated_MAP2K-KRAS_genes_dotPlot.pdf", onc.dot.plots, width=3, height=10)
ggplot2::ggsave("Correlated_MAP2K-KRAS_genes_dotPlot_25max.pdf", onc.dot.plots, width=3, height=13)
