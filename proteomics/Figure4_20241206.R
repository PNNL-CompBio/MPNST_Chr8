...
#### 1. volcano plot of # of GSEA_kinaseT_GTRD results ####
...
dot.plot <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    x = "Omics Type",
    y = "Kinase",
    color = "enrichment_value_log2", size = "-log(FDR)"
  ) + theme(axis.title.x=element_blank(),
           axis.text.x=element_blank(),
           axis.ticks.x=element_blank())
dot.plot
# most are only in prot and not in RNA
ggplot2::ggsave("Enriched_kinases_dotPlot.pdf", dot.plot, width=3, height=4)
...
#topGenes <- unique(dot.df[order(-dot.df$enrichment_value_log2),]$kinase)
topGenes <- rev(geneOrder)
topGeneSets <- rev(dot.df[order(dot.df$enrichment_value_log2),]$kinase)
corr.result <- read.csv(synapser::synGet("syn64024300")$path)
corr.result <- na.omit(corr.result[corr.result$type=="RNA" & corr.result$feature %in% tf.genes,]) # 205
if (any(corr.result$Spearman.q == 0)) {
  corr.result[corr.result$Spearman.q == 0,]$Spearman.q <- 0.0001
}
corr.result$adjusted_p_value_log10_abs <- -log(corr.result$Spearman.q, base = 10)
maxAbsEst <- max(abs(corr.result$Spearman.est))
maxLogFDR <- max(abs(corr.result$adjusted_p_value_log10_abs))
...
for (i in 1:length(topGeneSets)) {
  ...
  temp.topGenes <- unique(temp.gene.corr[temp.gene.corr$Significance == paste0("FDR > ", FDR),]$feature) # 48
  ...
  if (i == 1) {
    gsea.dot.plots <- (dot.plot + theme(legend.position = "none"))
  } else if (i == ceiling(length(topGeneSets)/2)) {
...
gsea.dot.plots
ggplot2::ggsave("Correlated_kinaseTs_dotPlot_patchworkCollection_minN6_tall_v2.pdf", gsea.dot.plots, width=9, height=16)
ggplot2::ggsave("Correlated_kinaseTs_dotPlot_patchworkCollection_minN6_v2.pdf", gsea.dot.plots, width=9, height=9)
