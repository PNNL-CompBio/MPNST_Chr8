...
contrasts <- c("Chr8_strongly_amplified")
outliers <- list("Proteomics" = c("X4", "X12"), #corresponds to JH-2-079c
                 "RNA-Seq" = list("PDX" = c("WU-356", "JH-2-031"),
                                  "Tumor" = "JH-2-023"),
                 "RNA-Seq (After Missingness Filter)" = list("PDX" = c("WU-356", "JH-2-031", "WU-368"),
                                  "Tumor" = "JH-2-023"),
                 "Proteomics (Old Database)" = c("X4", "X12"))
#Chr8.match.ids <- global.meta.df[global.meta.df$common_name %in% Chr8.matching,]$id
Chr8.match.ids <- global.meta.df2[global.meta.df2$common_name != "MN-2",]$id
Chr8.matching <- global.meta.df2[global.meta.df2$common_name != "MN-2" &
                                   global.meta.df2$common_name != "JH-2-023" &
                                   global.meta.df2$common_name != "WU-386" & 
                                   global.meta.df2$common_name != "WU-486",]$common_name
...
global.meta.df2[global.meta.df2$PDX_Chr8_status_BG == "Strongly amplified", ]$Chr8_inclusive <- "Strongly_amplified"
...
pca.meta.df[pca.meta.df$PDX_Chr8_status_BG == "Strongly amplified", ]$Chr8_inclusive <- "Strongly_amplified"
  ...
  for (i in 1:length(omics)) {
    ...
    if (is.list(outliers[[i]])) {
      for (k in 1:length(outliers[[i]])) {
        ...
        meta.wo.out <- meta.list[[i]][!(meta.list[[i]]$id %in% temp.out),]
