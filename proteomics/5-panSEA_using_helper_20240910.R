...
setwd("~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/")
dir.create("PCSF_directional_20240930_5-100")
setwd("PCSF_directional_20240930_5-100")
base.path <- getwd()
for (i in n.top) {
  setwd(base.path)
  ...
  for (j in directions) {
    ...
    for (k in 1:length(inputs)) {
      ...
      # run PCSF
      temp.fname <- paste0("PCSF_Chr8_", names(inputs)[k], "_top_", i, "_sig_", j)
      if (length(final.inputs) == 3) {
        pcsf.result <- computePhosphoNetwork(phos.vals = unlist(final.inputs[[1]]), 
