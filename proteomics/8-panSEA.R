...
global.stricter <- global.df[ , meta.df[meta.df$SampleID != "JH-2-002", ]$Tube]
global.set <- MSnSet(exprs = global.stricter %>% as.matrix(),
                     pData = meta.df[meta.df$SampleID != "JH-2-002", ])
plot_pca(global.set, phenotype = "SampleID") + ggtitle("Global PCA")
...
# samples except JH-2-002
global.w.mouse.stricter <- global.w.mouse.df[ , meta.df[meta.df$SampleID != "JH-2-002", ]$Tube]
...
stricter.meta.df <- meta.df[meta.df$SampleID != "JH-2-002",]

### DMEA
data.list <- list(global.df, global.w.mouse.df)
types <- c("Proteomics", "PDX Proteomics")
Chr8.prot.samples <- paste0("X", stricter.meta.df[stricter.meta.df$Chr8_amp & 
                                                    stricter.meta.df$Sample == "Sample", ]$Tube)
non.Chr8.prot.samples <- paste0("X", stricter.meta.df[!stricter.meta.df$Chr8_amp & 
                                                        stricter.meta.df$Sample == "Sample", ]$Tube)
DMEA3 <- panSEA::panSEA(data.list, types, GSEA = FALSE, 
                        group.names = c("Chr8_amp","not_amp"),
                        group.samples = list(Chr8.prot.samples,
