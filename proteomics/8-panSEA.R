...
phospho.venn <- phospho.df  %>% tidyr::extract(SUB_SITE, "Gene",
                                               remove = FALSE)
venn.list <- list('Proteomics wo mouse' = unique(global.df$Gene),
                  'Proteomics' = unique(global.w.mouse.df$Gene),
                  'RNA-Seq' = unique(RNA.df$Gene),
                  'Phospho wo mouse' = unique(phospho.venn$Gene))
ggvenn::ggvenn(venn.list, set_name_size = 4)
ggplot2::ggsave("Chr8_venn_diagram_20240216.pdf")
...
ggplot2::ggsave("Chr8_venn_diagram_20240216v3.pdf")
...
pca.global.df <- global.df
pca.global.w.mouse.df <- global.w.mouse.df
pca.phospho.df <- phospho.df
colnames(pca.global.df) <- seq(1, ncol(pca.global.df))
colnames(pca.global.w.mouse.df) <- seq(1, ncol(pca.global.w.mouse.df))
colnames(pca.phospho.df) <- seq(1, ncol(pca.phospho.df))

# add Chr8 status to metadata
meta.df$Chr8_status <- "Not amplified"
meta.df[meta.df$Sample == "Reference" | 
          meta.df$Chr8_amp, ]$Chr8_status <- "Amplified"
...
meta.stricter <- meta.df[meta.df$SampleID != "JH-2-002", ]
rownames(meta.stricter) <- paste0("X", meta.stricter$Tube)
...
# samples except JH-2-002
global.stricter <- global.df[ , rownames(meta.stricter)]
global.set <- MSnSet(exprs = global.stricter %>% as.matrix(),
                     pData = meta.stricter)
plot_pca(global.set, phenotype = "SampleID") + ggtitle("Global PCA")
ggsave("Global_PCA_betterSampleID_KnownChr8StatusOnly_stricter.pdf")

plot_pca(global.set, phenotype = "Chr8_status") + ggtitle("Global PCA")
ggsave("Global_PCA_byChr8Status_KnownChr8StatusOnly_stricter.pdf")
...
# samples except JH-2-002
global.w.mouse.stricter <- global.w.mouse.df[ , rownames(meta.stricter)]
global.w.mouse.set <- MSnSet(exprs = global.w.mouse.stricter %>% as.matrix(),
                     pData = meta.stricter)
plot_pca(global.w.mouse.set, phenotype = "SampleID") + ggtitle("Global PCA")
ggsave("Global_PDX_PCA_betterSampleID_KnownChr8StatusOnly_stricter.pdf")

plot_pca(global.w.mouse.set, phenotype = "Chr8_status") + ggtitle("Global PCA")
ggsave("Global_PDX_PCA_byChr8Status_KnownChr8StatusOnly_stricter.pdf")
...
# samples except JH-2-002
phospho.stricter <- phospho.df[ , rownames(meta.stricter)]
phospho.set <- MSnSet(exprs = phospho.stricter %>% as.matrix(),
                      pData = meta.stricter)
plot_pca(phospho.set, phenotype = "SampleID") + ggtitle("Phospho PCA")
ggsave("Phospho_PDX_PCA_betterSampleID_KnownChr8StatusOnly_stricter.pdf")

plot_pca(phospho.set, phenotype = "Chr8_status") + ggtitle("Phospho PCA")
ggsave("Phospho_PDX_PCA_byChr8Status_KnownChr8StatusOnly_stricter.pdf")
...
setwd("~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/data/global_with_mouse_data/analysis/")
saveRDS(panSEA1_C1GSEA_custom, file=paste0("Chr8_", "RNAseq_PDX", "_GSEA_positional_custom.rds"))
saveRDS(panSEA1_C1GSEA, file=paste0("Chr8_", "RNAseq_PDX", "_GSEA_positional.rds"))
saveRDS(panSEA1_hallmarkGSEA, file=paste0("Chr8_", "RNAseq_and_global_PDX", "_GSEA_hallmark.rds"))
saveRDS(panSEA1, file=paste0("Chr8_", "RNAseq_and_global_PDX", "_panSEA_CCLE.rds"))
saveRDS(DMEA1_prot, file=paste0("Chr8_", "RNAseq_and_global_PDX", "_DMEA_CCLE_proteomics.rds"))
...
#### 4. Run panSEA: all MPNST samples w known Chr8 status ####
### DMEA
...
#### 5. Run panSEA: re-do proteomics w all MPNST PDX samples w known Chr8 status except JH-2-002 ####
# remove JH-2-002 from further consideration because not strongly Chr8-amplified
### DMEA
global.stricter$Gene <- rownames(global.stricter)
global.w.mouse.stricter$Gene <- rownames(global.w.mouse.stricter)
phospho.stricter$SUB_SITE <- rownames(phospho.stricter)

data.list <- list(global.stricter, global.w.mouse.stricter)
types <- c("Proteomics (new database)", "Proteomics")
Chr8.prot.samples <- paste0("X", meta.stricter[meta.stricter$Chr8_amp, ]$Tube)
non.Chr8.prot.samples <- paste0("X", meta.stricter[!meta.stricter$Chr8_amp, ]$Tube)
DMEA3 <- panSEA::panSEA(data.list, types, GSEA = FALSE, 
                        group.names = c("Chr8_amp","not_amp"),
                        group.samples = list(Chr8.prot.samples,
                                             non.Chr8.prot.samples))

data.list <- list("Global" = global.w.mouse.stricter, "Phospho" = phospho.stricter)
types <- c("Proteomics", "Phospho")
meta.stricter$Chr8 <- sub(" ", "_", meta.stricter$Chr8_status)
meta.stricter$id <- rownames(meta.stricter)
#omics <- list("Global" = global.w.mouse.stricter, "Phospho" = phospho.stricter)
gmt.sub <- readRDS(paste0(
  base.path,"phospho_data/gmt_PNNL_kinase-substrate_Chr8_5344.rds"))
# run_contrasts_global_phospho(list(c("Amplified", "Not amplified")), "Chr8", "id",
#                              meta.stricter, omics, )
...
gmt.ksdb <- readRDS(file.path(base.path,"gmt_ksdb_human_PNNL.rds"))
GSEA3_hallmark <- panSEA::panSEA(data.list, types, 
                                 DMEA = FALSE, feature.names = c("Gene", "SUB_SITE"),
                                 group.names = c("Chr8_amp","not_amp"),
                                 group.samples = list(Chr8.prot.samples,
                                                      non.Chr8.prot.samples),
                                 gmt.features = list("msigdb_Homo sapiens_H",
                                                     gmt.ksdb))
# GSEA3_positional <- panSEA::panSEA(data.list[1], "Proteomics",
#                                  DMEA = FALSE,
#                                  group.names = c("Chr8_amp","not_amp"),
#                                  group.samples = list(Chr8.prot.samples,
#                                                       non.Chr8.prot.samples),
...
# gmt$geneset.descriptions[[length(gmt$geneset.descriptions)+1]] <- "Chr8 cancer-associated genes"
# 
# GSEA3_positional_custom <- panSEA::panSEA(data.list[1], "Proteomics", 
#                                           DMEA = FALSE,
#                                           group.names = c("Chr8_amp","not_amp"),
#                                           group.samples = list(Chr8.prot.samples,
#                                                                non.Chr8.prot.samples),
...
global.DEG.files <- list("Differential_expression_results.csv" = 
                           DMEA3$mDEG.results$all.results$'Proteomics (new database)')
global.w.mouse.DEG.files <- list("Differential_expression_results.csv" = 
                            DMEA3$mDEG.results$all.results$Proteomics)
phospho.DEG.files <- list("Differential_expression_results.csv" = 
                            GSEA3$mDEG.results$all.results$Phospho)

GSEA.mtn.files <- list("GSEA_mountain_plot_KEGG_COMPLEMENT_AND_COAGULATION_CASCADES.pdf" =
                         GSEA3$mGSEA.results$all.results$'Proteomics'$mtn.plots[["KEGG_COMPLEMENT_AND_COAGULATION_CASCADES"]],
                       "GSEA_mountain_plot_KEGG_SYSTEMIC_LUPUS_ERYTHEMATOSUS.pdf" =
                         GSEA3$mGSEA.results$all.results$'Proteomics'$mtn.plots[["KEGG_SYSTEMIC_LUPUS_ERYTHEMATOSUS"]])
GSEA.files <- list("GSEA_results.csv" =
                     GSEA3$mGSEA.results$all.results$Proteomics$result,
                   "GSEA_volcano_plot.pdf" =
                     GSEA3$mGSEA.results$all.results$'Proteomics'$volcano.plot,
                   "mtn_plots" = GSEA.mtn.files
                   )

data.list <- list("Substrate_enrichment" = GSEA3$mGSEA.results$all.results$Phospho$result,
                  "KSEA" = GSEA3_hallmark$mGSEA.results$all.results$Phospho$result)
# phospho.GSEA3 <- panSEA::panSEA(data.list, names(data.list), DMEA = FALSE, 
#                         feature.names = rep("Feature_set", length(data.list)),
#                         group.names = "Chr8_amp_vs_not_amp", 
#                         GSEA.rank.var = rep("NES", length(data.list)),
#                         group.samples = 4) # NES is in column 4
# phospho.GSEA3_hallmark <- panSEA::panSEA(data.list, names(data.list), DMEA = FALSE, 
#                                 feature.names = rep("Feature_set", length(data.list)),
#                                 group.names = "Chr8_amp_vs_not_amp",
#                                 group.samples = 4, 
#                                 GSEA.rank.var = rep("NES", length(data.list)),
#                                 gmt.features = list("msigdb_Homo sapiens_H",
#                                                     "msigdb_Homo sapiens_H")) # NES is in column 4

phospho.GSEA3 <- panSEA::panSEA(data.list[1], names(data.list[1]), DMEA = FALSE, 
                                feature.names = rep("Feature_set", length(data.list[1])),
                                group.names = "Chr8_amp_vs_not_amp", 
                                GSEA.rank.var = rep("NES", length(data.list[1])),
                                group.samples = 4) # NES is in column 4
phospho.GSEA3_hallmark <- panSEA::panSEA(data.list[1], names(data.list[1]), DMEA = FALSE, 
                                         feature.names = rep("Feature_set", length(data.list[1])),
                                         group.names = "Chr8_amp_vs_not_amp",
                                         group.samples = 4, 
                                         GSEA.rank.var = rep("NES", length(data.list[1])),
                                         gmt.features = list("msigdb_Homo sapiens_H")) # NES is in column 4

sub.KEGG.mtn <- get_top_mtn_plots(phospho.GSEA3$mGSEA.results$NES$all.results$Substrate_enrichment, 
                             EA.type = "Substrate_enrichment")
sub.KEGG.files <- list("Substrate_enrichment_interactive_network_graph.html" =
                    phospho.GSEA3$mGSEA.network$NES$interactive,
                  "Substrate_enrichment_volcano_plot.pdf" =
                    phospho.GSEA3$mGSEA.results$NES$all.results$'Substrate_enrichment'$volcano.plot,
                  "Substrate_enrichment_results.csv" =
                    phospho.GSEA3$mGSEA.results$NES$all.results$Substrate_enrichment$result,
                  "mtn_plots" = sub.KEGG.mtn)
sub.H.files <- list("Substrate_enrichment_volcano_plot.pdf" =
                         phospho.GSEA3_hallmark$mGSEA.results$NES$all.results$'Substrate_enrichment'$volcano.plot,
                       "Substrate_enrichment_results.csv" =
                         phospho.GSEA3_hallmark$mGSEA.results$NES$all.results$Substrate_enrichment$result)
sub.GSEA.files <- list("GSEA_KEGG" = sub.KEGG.files,
                       "GSEA_HALLMARK" = sub.H.files) 

sub.mtn <- get_top_mtn_plots(GSEA3$mGSEA.results$all.results$Phospho, 
                             EA.type = "Substrate_enrichment")
sub.files <- list("Substrate_enrichment_interactive_network_graph.html" =
                     GSEA3$mGSEA.network$interactive,
                   "Substrate_enrichment_volcano_plot.pdf" =
                     GSEA3$mGSEA.results$all.results$'Phospho'$volcano.plot,
                  "Substrate_enrichment_results.csv" =
                    GSEA3$mGSEA.results$all.results$Phospho$result,
                  "mtn_plots" = sub.mtn,
                  "GSEA" = sub.GSEA.files)

kin.mtn <- get_top_mtn_plots(GSEA3_hallmark$mGSEA.results$all.results$Phospho, 
                             EA.type = "KSEA")
kin.files <- list("KSEA_interactive_network_graph.html" =
                    GSEA3_hallmark$mGSEA.network$interactive,
                  "KSEA_volcano_plot.pdf" =
                    GSEA3_hallmark$mGSEA.results$all.results$'Phospho'$volcano.plot,
                  "KSEA_results.csv" =
                    GSEA3_hallmark$mGSEA.results$all.results$Phospho$result,
                  "mtn_plots" = kin.mtn)
phospho.files <- list("Differential_expression" = phospho.DEG.files,
                      "KSEA" = kin.files,
                      "Substrate_enrichment" = sub.files)

GSEA.H.mtn.files <- list("GSEA_mountain_plot_HALLMARK_COAGULATION.pdf" = 
                           GSEA3_hallmark[["mGSEA.results"]][["all.results"]][["Proteomics"]][["mtn.plots"]][["HALLMARK_COAGULATION"]])
GSEA.H.files <- list("GSEA_results.csv" =
                       GSEA3_hallmark$mGSEA.results$all.results$'Proteomics'$result,
                     "GSEA_interactive_network.graph.html" =
                       GSEA3_hallmark$mGSEA.network$interactive,
                     "GSEA_volcano_plot.pdf" =
                       GSEA3_hallmark$mGSEA.results$all.results$'Proteomics'$volcano.plot,
                     "mtn_plots" = GSEA.H.mtn.files)
...
combo.DMEA.files <- list("DMEA_results.csv" =
                   ...
                   "DMEA_interactive_network.graph.html" =
                     DMEA3$mDMEA.network$interactive)
global.w.mouse.DMEA.mtn <- get_top_mtn_plots(DMEA3$mDMEA.results$all.results$Proteomics, EA.type = "DMEA", sets = "Drug_set")
global.w.mouse.DMEA.files <- list("DMEA_results.csv" =
                     DMEA3$mDMEA.results$all.results$Proteomics$results,
                   "DMEA_volcano_plot.pdf" =
                     DMEA3$mDMEA.results$all.results$Proteomics$volcano.plot,
                   "DMEA_correlation_results.csv" =
                     DMEA3$mDMEA.results$all.results$Proteomics$corr.result,
                   "DMEA_correlation_scatter_plots.pdf" =
                     DMEA3$mDMEA.results$all.results$'Proteomics'$corr.scatter.plots,
                   "mtn_plots" = global.w.mouse.DMEA.mtn)
global.DMEA.mtn <- get_top_mtn_plots(DMEA3$mDMEA.results$all.results$'Proteomics (new database)', EA.type = "DMEA", sets = "Drug_set")
global.DMEA.files <- list("DMEA_results.csv" =
                                    DMEA3$mDMEA.results$all.results$'Proteomics (new database)'$results,
                                  "DMEA_volcano_plot.pdf" =
                                    DMEA3$mDMEA.results$all.results$'Proteomics (new database)'$volcano.plot,
                                  "DMEA_correlation_results.csv" =
                                    DMEA3$mDMEA.results$all.results$'Proteomics (new database)'$corr.result,
                                  "DMEA_correlation_scatter_plots.pdf" =
                                    DMEA3$mDMEA.results$all.results$'Proteomics (new database)'$corr.scatter.plots,
                                  "mtn_plots" = global.DMEA.mtn)

combo.DMEA.prot.files <- list("DMEA_results.csv" =
                           DMEA3_prot$mDMEA.results$compiled.results$results,
                         "DMEA_mean_results.csv" =
                           DMEA3_prot$mDMEA.results$compiled.results$mean.results,
                         "DMEA_correlation_matrix.pdf" =
                         ...
                         "DMEA_interactive_network.graph.html" =
                           DMEA3_prot$mDMEA.network$interactive)
global.w.mouse.DMEA.prot.mtn <- get_top_mtn_plots(DMEA3_prot$mDMEA.results$all.results$Proteomics, EA.type = "DMEA", sets = "Drug_set")
global.w.mouse.DMEA.prot.files <- list("DMEA_results.csv" =
                                    DMEA3_prot$mDMEA.results$all.results$Proteomics$results,
                                  "DMEA_volcano_plot.pdf" =
                                    DMEA3_prot$mDMEA.results$all.results$Proteomics$volcano.plot,
                                  "DMEA_correlation_results.csv" =
                                    DMEA3_prot$mDMEA.results$all.results$Proteomics$corr.result,
                                  "DMEA_correlation_scatter_plots.pdf" =
                                    DMEA3_prot$mDMEA.results$all.results$'Proteomics'$corr.scatter.plots,
                                  "mtn_plots" = global.w.mouse.DMEA.prot.mtn)
global.DMEA.prot.mtn <- get_top_mtn_plots(DMEA3_prot$mDMEA.results$all.results$'Proteomics (new database)', EA.type = "DMEA", sets = "Drug_set")
global.DMEA.prot.files <- list("DMEA_results.csv" =
                            DMEA3_prot$mDMEA.results$all.results$'Proteomics (new database)'$results,
                          "DMEA_volcano_plot.pdf" =
                            DMEA3_prot$mDMEA.results$all.results$'Proteomics (new database)'$volcano.plot,
                          "DMEA_correlation_results.csv" =
                            DMEA3_prot$mDMEA.results$all.results$'Proteomics (new database)'$corr.result,
                          "DMEA_correlation_scatter_plots.pdf" =
                            DMEA3_prot$mDMEA.results$all.results$'Proteomics (new database)'$corr.scatter.plots,
                          "mtn_plots" = global.DMEA.prot.mtn)
combo.files <- list("Differential_expression" = combo.DEG.files,
                    "DMEA" = combo.DMEA.files,
                    "DMEA_prot" = combo.DMEA.prot.files)
global.files <- list("Differential_expression" = global.DEG.files,
                     "DMEA" = global.DMEA.files,
                     "DMEA_prot" = global.DMEA.prot.files)
global.w.mouse.files <- list("Differential_expression" = global.w.mouse.DEG.files,
                             "GSEA_KEGG" = GSEA.files,
                             "GSEA_hallmark" = GSEA.H.files,
                             "DMEA" = global.w.mouse.DMEA.files,
                             "DMEA_prot" = global.w.mouse.DMEA.prot.files)
all.files <- list('global' = global.w.mouse.files,
                  'global_new_database' = global.files,
                  'global_new_and_old_database' = combo.files,
...
base.path <- "~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/analysis/stricter"
setwd(base.path)
contrastFolder <- "syn53632578"
synapser::synLogin()
save_contrast_to_synapse(all.files, contrastFolder, "Chr8_amp_vs_not_amp")

#### 5a. Run panSEA: re-do proteomics w all MPNST PDX samples w known Chr8 status except JH-2-002 and MN-2 ####
# remove JH-2-002 and MN-2 from further consideration because not strongly Chr8-amplified
meta.evenStricter <- meta.stricter[meta.stricter$SampleID != "MN-2", ]
### DMEA
global.evenStricter <- global.df[ , c("Gene", rownames(meta.evenStricter))]
global.evenStricter$Gene <- rownames(global.evenStricter)

global.w.mouse.evenStricter <- global.w.mouse.df[ , c("Gene", rownames(meta.evenStricter))]
global.w.mouse.evenStricter$Gene <- rownames(global.w.mouse.evenStricter)

phospho.evenStricter <- phospho.df[ , c("SUB_SITE", rownames(meta.evenStricter))]
phospho.evenStricter$SUB_SITE <- rownames(phospho.evenStricter)

data.list <- list(global.evenStricter, global.w.mouse.evenStricter)
types <- c("Proteomics (new database)", "Proteomics")
Chr8.prot.samples <- paste0("X", meta.evenStricter[meta.evenStricter$Chr8_amp, ]$Tube)
non.Chr8.prot.samples <- paste0("X", meta.evenStricter[!meta.evenStricter$Chr8_amp, ]$Tube)
DMEA3 <- panSEA::panSEA(data.list, types, GSEA = FALSE, 
                        group.names = c("Chr8_amp","not_amp"),
                        group.samples = list(Chr8.prot.samples,
                                             non.Chr8.prot.samples))

data.list <- list("Global" = global.w.mouse.evenStricter, "Phospho" = phospho.evenStricter)
types <- c("Proteomics", "Phospho")
meta.evenStricter$Chr8 <- sub(" ", "_", meta.evenStricter$Chr8_status)
meta.evenStricter$id <- rownames(meta.evenStricter)
#omics <- list("Global" = global.w.mouse.evenStricter, "Phospho" = phospho.evenStricter)
base.path1 <- "~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/data/"
gmt.sub <- readRDS(paste0(
  base.path1,"phospho_data/gmt_PNNL_kinase-substrate_Chr8_5344.rds"))
base.path <- "~/OneDrive - PNNL/Documents/GitHub/Chr8/proteomics/analysis/evenStricter"
# run_contrasts_global_phospho(list(c("Amplified", "Not amplified")), "Chr8", "id",
#                              meta.evenStricter, omics, )
...
gmt.ksdb <- readRDS(file.path(base.path1,"gmt_ksdb_human_PNNL.rds"))
GSEA3_hallmark <- panSEA::panSEA(data.list, types, 
                                 DMEA = FALSE, feature.names = c("Gene", "SUB_SITE"),
                                 group.names = c("Chr8_amp","not_amp"),
                                 group.samples = list(Chr8.prot.samples,
...
sub.mtn <- get_top_mtn_plots(GSEA3$mGSEA.results$all.results$Phospho, n.top = 20,
                             EA.type = "Substrate_enrichment")
sub.files <- list("Substrate_enrichment_interactive_network_graph.html" =
                     GSEA3$mGSEA.network$interactive,
                   "Substrate_enrichment_volcano_plot.pdf" =
...
kin.mtn <- get_top_mtn_plots(GSEA3_hallmark$mGSEA.results$all.results$Phospho, n.top=20,
                             EA.type = "KSEA")
kin.files <- list("KSEA_interactive_network_graph.html" =
                    GSEA3_hallmark$mGSEA.network$interactive,
                  "KSEA_volcano_plot.pdf" =
...
contrastFolder <- NULL
#synapser::synLogin()
save_contrast_to_synapse(all.files, contrastFolder, "Chr8_amp_vs_not_amp", base.path)
### DMEA
global.evenStricter$Gene <- rownames(global.evenStricter)
global.w.mouse.evenStricter$Gene <- rownames(global.w.mouse.evenStricter)
phospho.evenStricter$SUB_SITE <- rownames(phospho.evenStricter)
